...
export function basicTests() {
	...
	describe('Start/Stop AtlasMap and verify correct notifications', () => {
		...
		it('Start Command should show a notification with the correct text', async function () {
			...
			await startAtlasMap();
			driver.sleep(1000);
			const notificationStarting = await driver.wait(() => { return getNotificationWithMessage(notifications.ATLASMAP_STARTING); }, 30000) as Notification;
			expect(await notificationStarting.getMessage()).contains(notifications.ATLASMAP_STARTING);
			await driver.wait(() => { return whilegetNotificationWithMessage(notifications.ATLASMAP_WAITING); }, 30000);
			driver.sleep(1000);
